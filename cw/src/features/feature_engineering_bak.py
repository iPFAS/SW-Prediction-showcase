...
class FeatureEngineering:
    ...
    def fit(self, df: pd.DataFrame) -> None:
        ...
        country_stats = df.groupby('Country Name').agg({
            ...
            'Population': 'mean',
            'GDP PPP 2017': 'mean',
            'Urban population %': 'mean'  # 添加城市化率的统计
        })
        ...
        urban_quantiles = country_stats['Urban population %'].quantile([0.2, 0.4, 0.6, 0.8]).to_dict()  # 城市化率分位数
        ...
        yearly_stats = df.groupby('Year').agg({
            ...
            'Urban population %': ['mean', 'std', 'median']  # 添加城市化率的年度统计
        })
        ...
        self.global_stats = {
            ...
            'urban_quantiles': urban_quantiles,  # 添加城市化率分位数
    ...
    def transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 1. 基础指标的非线性特征
        for metric in ['GDP PPP 2017', 'GDP PPP/capita 2017', 'Population', 'Urban population %']:  # 添加城市化率
            metric_name = metric.lower().replace(' ', '_').replace('/', '_per_').replace('%', 'pct')
            
            # 对数变换
            if metric != 'Urban population %':  # 城市化率已经是百分比，不需要对数变换
                df[f'{metric_name}_log'] = np.log1p(df[metric])
            else:
                df[f'{metric_name}_log'] = df[metric] / 100  # 转换为0-1之间的比例
            ...
            if metric != 'Urban population %':
                ...
                df[f'{metric_name}_exp'] = np.exp(df[f'{metric_name}_log']) - 1  # 城市化率不需要除以10
            ...
            # 库兹涅茨曲线特征 - 针对人均GDP和城市化率特别有意义
            if 'capita' in metric_name or 'urban' in metric_name:
                # 倒U型变换 - 模拟环境库兹涅茨曲线
        ...
        if self.global_stats:
            ...
            # 使用预先计算的城市化率分位数
            urban_quantiles = self.global_stats.get('urban_quantiles', {})
            if urban_quantiles:
                df['global_urbanization_stage'] = pd.cut(
                    df['Urban population %'],
                    bins=[-np.inf] + list(urban_quantiles.values()) + [np.inf],
                    labels=range(5)
                ).fillna(0).astype(int)
            ...
            # 计算与全球基准的相对位置
            for metric in ['GDP PPP/capita 2017', 'Population', 'GDP PPP 2017', 'Urban population %']:  # 添加城市化率
                metric_name = metric.lower().replace(' ', '_').replace('/', '_per_').replace('%', 'pct')
            ...
            df['global_urbanization_stage'] = df.groupby('Year')['Urban population %'].transform(
                lambda x: pd.qcut(x, 4, labels=False, duplicates='drop')
            ).fillna(0).astype(int)
        ...
        # 添加建筑垃圾与城市化相关的交互特征
        # 城市化率与GDP的交互特征 - 对建筑垃圾产生量影响显著
        df['urban_gdp_interaction'] = df['Urban population %'] * df['GDP PPP/capita 2017'] / 100
        
        # 城市化率与人口的交互 - 反映城市人口规模
        df['urban_population'] = df['Urban population %'] * df['Population'] / 100
        
        # 城市化率与收入组的交互 - 不同收入水平国家的城市化影响不同
        df['urban_income_interaction'] = df['Urban population %'] * df['income_group_ordinal'] / 100
        
        # 城市化率与区域的交互 - 不同区域的城市化模式不同
        df['urban_region_interaction'] = df['Urban population %'] * df['region_ordinal'] / 100
        
        # 城市化率变化与GDP增长的交互 - 捕捉城市化进程与经济发展的协同效应
        df['urban_gdp_growth_interaction'] = df['urban_population_pct_growth'] * df['gdp_ppp_per_capita_2017_growth']
        
        # 城市化率的库兹涅茨曲线特征 - 城市化率达到一定水平后，建筑垃圾产生模式可能变化
        urban_rate = df['Urban population %'] / 100
        urban_peak = 0.75  # 假设75%城市化率是拐点
        df['urban_ekc'] = -((urban_rate - urban_peak) ** 2)
        # 处理异常值和缺失值
